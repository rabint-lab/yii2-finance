...
use rabint\finance\models\WalletConnectionSearch;
...
/**
 * AdminWalletConnectionController implements the CRUD actions for WalletConnection model.
 */
class AdminWalletConnectionController extends \rabint\controllers\AdminController {
    ...
    public function actionBulk($action)
    {
        ...
        switch ($action) {
            case static::BULK_ACTION_SETPUBLISH:
                if (WalletConnection::updateAll(['status' => WalletConnection::STATUS_DRAFT], ['id' => $selection])) {
            ...
            case static::BULK_ACTION_DELETE:
                if (WalletConnection::deleteAll(['id' => $selection])) {
                    Yii::$app->session->setFlash('success',  Yii::t('rabint', 'Bulk action was successful'));
                } else {
                    $err++;
                }
    ...
    /**
     * Lists all WalletConnection models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new WalletConnectionSearch();
    ...
    /**
     * Displays a single WalletConnection model.
     * @param int $id شناسه
     * @return mixed
     */
    ...
    /**
     * Creates a new WalletConnection model.
            ...
            if ($model->save()) {
                ...
                if ($request->isAjax) {
                    ...
                    return [
                        ...
                        'title' =>  Yii::t('rabint', 'Create new').' '. Yii::t('rabint', 'WalletConnection'),
    ...
    /**
     * Updates an existing WalletConnection model.
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                ...
                if ($request->isAjax) {
                    ...
                    [
                        'forceReload' => '#ajaxCrudDatatable',
                        'title' =>  Yii::t('rabint', 'Updating').' '. Yii::t('rabint', 'WalletConnection'),
    ...
    /**
     * Deletes an existing WalletConnection model.
    ...
    /**
     * Finds the WalletConnection model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $id شناسه
     * @return WalletConnection the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = WalletConnection::findOne($id)) !== null) {
            return $model;
        } else {
